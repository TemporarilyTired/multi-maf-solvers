...
pub struct ArenaTree {
    pub arena: Vec<Node>,
    pub roots: HashSet<Idx>,
    pub leaf_map: HashMap<Label, Idx>,
}
...
impl ArenaTree {
    ...
    pub fn cut_possibly_contracted_or_root_branch(&mut self, mut cut: Idx) {
        assert_validity!(self);
        // descent to the uncut child until we are at an uncontrated node
        while self.get(cut).status == Contracted {
            let Internal { left, right } = self.get(cut).data else {
                unreachable!("leaf cannot be contracted");
            };
            if self.get(left).parent.is_some() {
                cut = left;
            } else if self.get(right).parent.is_some() {
                cut = right;
            } else {
                dbg!("warning: tried to cut branch without nodes (useless cut)");
                return;
            }
        }

        // find parent
        let Some(parent) = self.get(cut).parent else {
            dbg!("warning: tried to cut branch without a parent");
            return;
        };
        debug_assert_eq!(self.get(parent).status, Present);
        debug_assert_eq!(self.get(cut).status, Present);
    ...
    pub fn make_svt(&mut self, label: Label) {
        let leaf = self.locate_label(label);
    ...
    pub fn new_from_indexed_bin_tree(tree: &IndexedBinTree) -> ArenaTree {
        fn build(
            tree: &IndexedBinTree,
            nodes: &mut HashMap<Idx, Node>,
            leaf_map: &mut HashMap<Label, Idx>,
            parent: Option<Idx>,
            next_idx: &mut usize,
        ) -> Idx {
        ...
        let mut nodes_map: HashMap<Idx, Node> = HashMap::new();
        let mut leaf_map: HashMap<Label, Idx> = HashMap::new();

        let root_idx = build(tree, &mut nodes_map, &mut leaf_map, None, &mut 0);

        let mut roots: HashSet<Idx> = HashSet::new();
    ...
    pub fn assert_validity(&self) {
        ...
        let mut unique_labels: HashSet<Label> = HashSet::new();
        // let mut unique_nodes: HashSet<NodeIdx> = HashSet::new();
