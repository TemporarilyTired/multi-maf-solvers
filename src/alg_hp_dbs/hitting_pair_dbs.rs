...
// Perform global preprocessing, calling binary_search_dbs_fn on every produced cluster
pub fn solve_binary_search_dbs(instance: Instance) -> ArenaTree {
    reduce_and_solve(instance, &binary_search_dbs_fn)
}

// Finds an agreement forest of an instance;
// Performs binary search on the order of the MAF,
// calling cherry_dbs for each MAF(instance,k) test.
fn binary_search_dbs_fn(instance: Instance) -> ArenaTree {
    let approx_state = State {
        instance: instance.clone(),
        cut_opts: HashSet::new(),
    };
    let (upper_bound_af, mut lb) = calc_expensive_lb_and_ub(approx_state);
...
/// Finds an agreement forest of the instance with order at most k, that also satisfies the hitting
/// pair (cut-opt) constraints, if it exists.
///
/// This method implements the HP-DBS algorithm, which uses elements from the
/// depth-bounded search algorithm in [Shi et al., 2018] (see ./alg_dbs_lsi/mod.rs)
///
/// # Parameters
///
/// - `state` - Input instance.
/// - `k` - Maximum allowed order of the resulting agreement forest.
/// - `is_rr1_and_rr2_reduced` - stores whether the given instance is already fully rr1 and rr2 reduced, used to avoid unneeded reduction calls during recursion
///
/// # Returns
///
/// - `Some(agreement forest)` if such an agreement forest of order at most `k` exists.
...
pub fn cherry_dbs(mut state: State, k: usize, is_rr1_and_rr2_reduced: bool) -> Option<ArenaTree> {
    ...
    if !is_rr1_and_rr2_reduced {
        let (performed_reductions, n_removed_svts) = state.remove_svt_and_merge();
    ...
    if state.instance.ord() + 4 < k {
        // NOTE: Subtree cluster reduction
    ...
    // we loop, possibly performing multiple reductions, before having to perform a branch
    loop {
        #[cfg(feature = "assert_validity")]
        state.assert_cut_opts_exist();
        #[cfg(feature = "assert_validity")]
...
/// Finds a lower bound and upper bound on the order of optimal solutions to MAF including the
/// hitting pair (cut-opt) constraints.
///
/// This method nearly `simulates` a cherry_dbs call, but performs a cut from branch instead of
/// actually branching. Then a lower bound is obtained by counting the non-forced cuts.
///
/// # Parameters
///
/// - `state` - Input instance.
...
/// # Returns
///
/// An upper bound to the optimal solution, given as an agreement forest of the instance, and a
/// lower bound given as an integer.
...
// An instance of HP-DBS; an extension of an instance to MAF, including hitting pairs (cut-opts)
// forcing at least one of each pair to be SVT.
#[derive(Clone)]
pub struct State {
    pub instance: Instance,
    pub cut_opts: HashSet<(Label, Label)>,
}
...
impl State {
    ...
    pub fn rr_2_2_1_reduce(&mut self) -> bool {
        ...
        let mut labels_in_a_cut_opt = HashSet::new();
        for &(a, b) in self.cut_opts.iter() {
    ...
    pub fn remove_svt_and_merge(&mut self) -> (Vec<PerformedReduction>, usize) {
    ...
    pub fn get_max_cut_opt(&self) -> (Option<Label>, usize) {
        let mut degrees: HashMap<Label, usize> = HashMap::new();
        for &(a2, b2) in self.cut_opts.iter() {
            *degrees.entry(a2).or_default() += 1;
            *degrees.entry(b2).or_default() += 1;
        }
    ...
    // -- debug functions to verify validity of the instances, e.g., pre- and postconditions -- //
    ...
    pub fn assert_validity(&self) {
        ...
        let labels_f1: HashSet<_> = f1.iterate_all().labels().collect();
        assert_eq!(labels_f1.len(), self.instance.num_leaves);

        for f in self.instance.forests.iter() {
            let labels: HashSet<_> = f.iterate_all().labels().collect();
